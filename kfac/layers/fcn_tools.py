...
class _Linear(torch.nn.Module):
    ...
    def __init__(self, h_in, h_out):
        super().__init__()
        self.weight = torch.nn.Parameter(torch.randn(h_out, h_in))
        self.h_in = h_in
        self.h_out = h_out
    
    def __repr__(self):
        return f"Linear({self.h_in}->{self.h_out}"
...
class _Layer(torch.nn.Module):
    ...
    def __init__(self, h_in, h_out, act, var_in, var_out):
        super().__init__()
        self.linear = _Linear(h_in, h_out)
        self.act = act
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.act is not None:
            x = x / (self.h_in * self.var_in)**0.5
            x = self.linear(x)
            x = self.act(x)
            x = x * self.var_out**0.5
        else:
            w = x / (self.h_in * self.var_in / self.var_out)**0.5
            x = self.linear(x)
        return x
