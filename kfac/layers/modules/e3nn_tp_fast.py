...
class E3nnTPFastModuleHelper(ModuleHelper):
    ...
    def __init__(self, module: torch.nn.Module):
        ...
        """
        self.module = module
        self.cogen_a_factor = codegen_tensor_product_a_factor_fast(
    ...
    def a_factor_shape(self) -> tuple[int, int]:
        ...
        """
        #TODO: This is just for sync work assignment in distributed case for optimum performance.
        #      Can be used as a estimation for work load. We can compute an sum for multiple tensors JUST FOR NOW.
    ...
    for ins in instructions:
        ...
        if ins.connection_mode == 'uvw':
            if specialized_code and l1l2l3 == (0, 0, 0):
                ...
                g = g_path #/ sqrt(mul_ir_out.ir.dim)  # CG effect
...
def codegen_tensor_product_a_factor_fast(
        irreps_in1: o3.irreps,
        irreps_in2: o3.irreps,
        irreps_out: o3.irreps,
        shared_weights: bool=False,
    ...
    for ins in instructions:
        ...
        if ins.connection_mode == 'uvw':
            a_hat_in1 = x1.reshape(batch_numel, mul_ir_in1.mul, mul_ir_in1.ir.dim)
            a_hat_in1 = torch.sum(a_hat_in1, dim=2, keepdim=False) # sum_pre
            a_hat_a_hat_t_in1 = a_hat_in1.T @ (a_hat_in1 / batch_numel)

            a_hat_in2 = x2.reshape(batch_numel, mul_ir_in2.mul, mul_ir_in2.ir.dim)
            a_hat_in2 = torch.sum(a_hat_in2, dim=2, keepdim=False) # sum_pre
            a_hat_a_hat_t_in2 = a_hat_in2.T @ (a_hat_in2 / batch_numel)
        else:
            raise NotImplementedError("not supporting yet")
